...
const updateCarousel = (index) => {  if (!carouselTrack || carouselSlides.length === 0) return;
...
if (servicesTrack && serviceSlides.length > 0) {
    servicesPrev?.addEventListener("click", () => { updateServicesCarousel(activeService - 1); restartServicesTimer(); });
    servicesNext?.addEventListener("click", () => { updateServicesCarousel(activeService + 1); restartServicesTimer(); });
    window.addEventListener("resize", () => updateServicesCarousel(activeService));
    updateServicesCarousel(0);
    restartServicesTimer();
}
    if (!carouselTrack || carouselSlides.leng
